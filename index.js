...
let STATS = [["class_score", "DESC", ">="], ["workload", "", "<="], ["difficulty","","<="], ["enjoyment", "DESC", ">="], ["teacher_score", "DESC", ">="], ["grade", "DESC", ">="]]
...
function teacher_class_score(res) {
...
function overall_teacher_score(res) {
  return new Promise(function(resolve, reject) {
...
function get_stat_rank(stat, desc, arrow, res) {
  return new Promise(function(resolve, reject) {
...
function stat_category(stat, desc, arrow, res) {
  return new Promise(function(resolve, reject) {
...
function teacher_stat(stat, res) {
  return new Promise(function(resolve, reject) {
...
function overall_teacher_stat(stat, res) {
  return new Promise(function(resolve, reject) {
...
function median_overall_stat(stat, res) {
  return new Promise(function(resolve, reject) {
...
async function do_stats_functions(req, res, next) {
  for (let i = 0; i < STATS.length; i++) {
    // console.log(STATS[i])
    if (STATS[i][0] === "grade") {
      await teacher_grade_num(res)
      await teacher_grade_num_overall(res)
    }
    await get_stat_rank(STATS[i][0], STATS[i][1], STATS[i][2], res)
    await stat_category(STATS[i][0], STATS[i][1], STATS[i][2], res)
    await median_stat(STATS[i][0], res)
    if (STATS[i][0] === "class_score") {
      await teacher_class_score(res)
      await overall_teacher_score(res)
    }
    else {
      await teacher_stat(STATS[i][0], res)
      await overall_teacher_stat(STATS[i][0], res)
    }
    await median_overall_stat(STATS[i][0], res)
  }
  next()
}
...
function teacher_grade_num(res) {
  return new Promise(function(resolve, reject) {
...
function teacher_grade_num_overall(res) {
  return new Promise(function(resolve, reject) {
...
function get_class_list_and_desc(req, res, next) {
  pool.query('SELECT name, alt, id, description FROM classes;', function(e, r) {
    res.locals.class_list_and_desc = r
    next()
  })
}
...
function submit_class_feedback(req, res, next) {
  ...
  try {
    ...
    class_score = parseFloat(req.body.class_score)
    workload = parseFloat(req.body.workload)
    difficulty = parseFloat(req.body.difficulty)
    enjoyment = parseFloat(req.body.enjoyment)
    teacher_score = parseFloat(req.body.teacher_score)
    show_teacher = req.body.show_teacher === "on" ? true : false
...
function edit_class_feedback(req, res, next) {
  ...
  try {
    class_name = req.body.class_name
    class_id = req.body.class_id
    original_term = req.body.original_term
    term = req.body.term
    teacher = req.body.teacher
    class_score = parseFloat(req.body.class_score)
    workload = parseFloat(req.body.workload)
    ...
    grade = req.body.grade
    feedback = req.body.feedback
    delete_feedback = req.body.delete_feedback
...
function add_review_review(req, res, next) {
  if ('authenticated' in req.session) {
    if (!(req.body.class_id in CLASSES)) next()
...
function get_search_results(req, res, next) {
  ...
  pool.query(`SELECT name, alt, id FROM classes WHERE name LIKE ? OR id LIKE ? OR alt LIKE ?`, [text, text, text], function(e, r) {
    if (r.length == 1) {
      ...
      res.locals.search_total = r.length
      res.locals.search_results = r
      next()
    }
  })
}

function submit_new_class_info(req, res, next) {
  console.log(req.body)
  if (isNaN(res.locals.profile.id)) next()
  if (req.body.description.length > 2500) next()
  pool.query(`INSERT INTO class_info VALUES (?, ?, ?)`, [res.locals.profile.id, req.body.class_id, req.body.description], function(e, r) {
    // console.log(e, r)
    next()
  })
}
...
let stats_middleware = [do_stats_functions]
...
app.get('/class/:classID', [getProfileData].concat(base_middleware).concat(stats_middleware), function (req, res) {
...
app.get('/submit_class_info', [checkAuthentication, getProfileData, get_class_list_and_desc], function(req, res, next) {
  res.locals.class_list_and_desc.sort(function(a, b) {
    if (a.name === b.name) {
      return a.id < b.id ? -1 : 1
    }
    return a.name < b.name ? -1 : 1
  })
  res.render('submit_class_info', {"login_link": authorizationUri, "profile": res.locals.profile, "class_list": res.locals.class_list_and_desc});
})

app.post('/class_info_submit', [checkAuthentication, getProfileData, submit_new_class_info], function(req, res, next) {
  res.send()
})
